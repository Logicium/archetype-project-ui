...
import { siteConfig as buildTimeConfig, type ProjectSiteConfig } from '../config/site.config'
...
export const useSiteContentStore = defineStore('siteContent', () => {
  const config = ref<ProjectSiteConfig>(buildTimeConfig)
  ...
  async function hydrate() {
    ...
    try {
      ...
      config.value = deepMerge(buildTimeConfig, res.content) as ProjectSiteConfig
      hydrated.value = true
    } catch (e) {
