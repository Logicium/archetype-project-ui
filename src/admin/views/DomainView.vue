...
async function redeploy() {
  error.value = null; redeployStatus.value = ''; redeploying.value = true
  try {
    const r = await contentClient.redeploySite(siteId.value)
    redeployStatus.value = `Deployment triggered (${r.deploymentId})`
  } catch (e) { error.value = e instanceof Error ? e.message : String(e) }
  finally { redeploying.value = false }
}
onMounted(async () => {
...
<template>
  <section>
    ...
    <div class="redeploy-section">
      <h2>Deployment</h2>
      <button type="button" :disabled="!siteId || redeploying" @click="redeploy">
        {{ redeploying ? 'Triggering…' : 'Redeploy site' }}
      </button>
      <p v-if="redeployStatus" class="ok">{{ redeployStatus }}</p>
    </div>
...
.redeploy-section { margin-top: 2rem; }
